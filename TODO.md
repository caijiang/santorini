...
- [✅] 如果修改数组类资源（环境）无法成功 patch; 通过很server-side-apply实现;修改量较大 需要更多测试
- [ ] 重新引入 spring boot actuator
- [✅] 可以通过飞书等方式订阅服务状态变化
- [✅] 允许普通用户新增服务(会检测检测是否存在)，然后自动授予该用户最高权限
- [✅] 新增角色 ingress,获得该角色的用户自动获得管理 ingress 的权限
- [ ] 允许 backend 添加私库
- [✅] k8s的一些资源都给设置缓存自动清理的时间; 只能简易处理了
- [✅] 设置了 hpa后，可以获取当前的数值(列表)
- [✅] 飞书通道，可以选择获得该环境: 部署,服务离线,配置
- [✅] 🪲普通用户新增服务后，无法再度编辑
- [✅] 🪲无法部署没有公开端口的服务
- [✅] 优化流量处理
  1. 没必要继续走同步 HOST 这条路
  2. host,分为 流量中的host,即将添加的host
  3. 个人权限问题也无需看到所有的入口(除非暴露服务自身可见)